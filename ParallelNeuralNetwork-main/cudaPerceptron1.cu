...
public:
    ...
    double* cuda_forward_propagate(const std::vector<std::vector<double>>& inputs);
    void backpropagate_error(const std::vector<std::vector<double>>& input, const std::vector<std::vector<double>>& output);
    void update_weights(const std::vector<std::vector<double>>& input, double learningRate);
...
/* Forward pass */
__global__ void cuda_forward_layer(const double *input_activations, const double *weights, double *output, int M, int N)
{
    int g_node = (blockIdx.x * blockDim.x) + threadIdx.x;
    int node = threadIdx.x;

    for (int i =0; i<N; i++)
    {
        output[g_node] += input_activations[blockIdx.x * M + i]*weights[node*N+i];
    }
}

double* neural_network::cuda_forward_propagate(const std::vector<std::vector<double>>& inputs)
{
    //std::vector<vector<double>> input = inputActivations;
    int m = inputs.size(); int n = inputs[0].size();
    double *inputArr = (double*)malloc(m * n * sizeof(double));

    for(int i=0; i<m; i++)
    {
        for(int j=0; j<n; j++)
        {
            inputArr[i*n + j] = inputs[i][j];
        }
    }
    double *in = &inputArr[0];
    static int debug = 0;
    ...
    {
        in = &inputArr[0];
        numThreadsPerBlock = layers[layer].nodes.size();

        long inputSize = m * n * sizeof(double);
        long outputSize = m * layers[layer].nodes.size()*sizeof(double);
        ...
        err = cudaMalloc((void**)&D_weights, size*layers[layer].nodes.size());

        double *D_outputActivations = NULL;
        err = cudaMalloc((void**)&D_outputActivations, outputSize);
        cudaMemset(D_outputActivations, 0, outputSize);

        double hostWeights[inputs[0].size()*layers[layer].nodes.size()];
        //double *dst = D_weights;
        double *dst = hostWeights;
        for (int node = 0; node < layers[layer].nodes.size(); node++)
        {
        ...
        //dst = D_weights;
        cudaMemcpy(D_weights, hostWeights, size*layers[layer].nodes.size(), cudaMemcpyHostToDevice);

        cuda_forward_layer<<<m, numThreadsPerBlock>>>(D_input_activations, D_weights, D_outputActivations, m, n);
        double* activations = (double*)malloc(outputSize*m);
        err = cudaMemcpy(activations, D_outputActivations, outputSize, cudaMemcpyDeviceToHost);
        n = layers[layer].nodes.size();
        for(int lane = 0; lane<m; lane++)
        {
            for (int node = 0; node < n; node++)
            {
                layers[layer].nodes[node].activation = activations[lane*n + node] + layers[layer].nodes[node].bias;
                layers[layer].nodes[node].output = sigmoid(layers[layer].nodes[node].activation, false);
                inputArr[lane*n + node] = layers[layer].nodes[node].output;
            }
        }
...
void neural_network::backpropagate_error(const std::vector<std::vector<double>>& inputs, const std::vector<std::vector<double>>& expected)
{
        double *outputArr = cuda_forward_propagate(inputs);

        for(int i=0; i<layers[layers.size()-1].nodes.size();i++)
        {
            double error = 0;
            for(int lane=0; lane<inputs.size(); lane++)
            {
                error += expected[lane][layers.size()-1] - outputArr[lane * inputs.size() + layers.size()-1];
            }
            error = error/inputs.size();
            layers[layers.size()-1].nodes[layers.size()-1].grad = error * sigmoid(layers[layers.size()-1].nodes[layers.size()-1].output, true);
        }
...
void neural_network::update_weights(const std::vector<std::vector<double>>& input, double learningRate)
{
    for (int i = 0; i< input.size(); i++)
    {
        for (int layer = 0; layer < layers.size(); layer++)
...
int main(int argc, char** argv)
{
    ...
    else
    {
        for (int epoch = 0; epoch < config.numOfEpochs; epoch++)
        {
            std::cout << "Epoch " << (epoch+1) << " / " << config.numOfEpochs << std::endl;
            std::shuffle(trainData.begin(), trainData.end(), rng);
            for (int i = 0; i < trainData.size(); i += 32)
            {
                std::vector<std::vector<double>> input, expected;
                for(int j=i; j<i+32; j++)
                {
                    input.push_back(trainData[j][0]);
                    expected.push_back(trainData[j][1]);
                }
                nn.backpropagate_error(input, expected);
                nn.update_weights(input, config.learningRate);
    ...
    for (int i = 0; i < testData.size(); i++)
    {
        ...
        double *output = nn.cuda_forward_propagate({testData[i][0]});
