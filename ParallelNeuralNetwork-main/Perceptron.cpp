...
File containing code to train and test a Multi Layer Percepton model on MNIST image data
sequentially and parallely using OpenMP.

References:
https://github.com/BobbyAnguelov/NeuralNetwork/tree/master/Src/NeuralNetwork
https://github.com/mitesh1612/Parallel-Neural-Networks
*/
...
{
    std::vector<double> output = evaluate(input);
...
int main(int argc, char** argv)
{
    ...
    if(argc < 3){
        std::cout<<"input params required. usage ./executable [num_epochs] [is_parallel]"<<std::endl;
        return -1;
    }

    int num_epochs = std::stoi(argv[1]);
    int is_parallel = std::stoi(argv[2]);
    ...
    int numThreads = is_parallel ? omp_get_max_threads() : 1;
    int batchLength = 100;
    ...
    {
        std::vector<double> output = model.evaluate(testData[i][0]);
